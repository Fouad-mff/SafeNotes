...
public class NotesDbAdapter {
    ...
    public static final String KEY_ROWID = "_id"; // Colonne de numéro de ligne dans la BD.
    private static final String TAG = "NotesDbAdapter";
    private DatabaseHelper mDbHelper;
    private SQLiteDatabase mDb;
    private static final String DATABASE_CREATE = "create table notes (_id integer primary key autoincrement, "
    											+ "title text not null, body text not null, date text not null);";
    			// Création d'une table "notes" dans la BD contenant les colonnes title, body et date.
    ...
    public long createNote(String title, String body, String date) {
    			// Création d'une nouvelle note Dans la BD.
    ...
    public boolean deleteNote(long rowId) {
    			// Suppression de la note à partir de la BD.
        return mDb.delete(DATABASE_TABLE, KEY_ROWID + "=" + rowId, null) > 0;
    }

    public Cursor fetchAllNotes() {
    			// Obtenir toutes les notes enregistées dans la BD.
        return mDb.query(DATABASE_TABLE, new String[] {KEY_ROWID,KEY_TITLE ,
        		KEY_BODY,KEY_DATE}, null, null, null, null, null);
    }

    public Cursor fetchNote(long rowId) throws SQLException {
    			// Obtenir la note dont le numéro de ligne dans la BD est "rowId".
    ...
    public boolean rename(long rowId, String title) {
    			// Renommer la note dont le numéro de ligne dans la BD est "rowId".
    ...
    public boolean updateNote(long rowId, String title, String body,String date) {
    			// Enregistrer les modifications effectuées à la note dans l'éditeur.
        ContentValues args = new ContentValues();
        args.put(KEY_TITLE, title);
